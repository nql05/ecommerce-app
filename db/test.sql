...
(
	...
	Gender BIT,
	-- Bit in SQL Server = Boolean in other DBMS
	BirthDate DATE,
	--- Format: YYYY-MM-DD
...
(
	...
	-- PhoneNumber contains 10 digits
	...
	-- Home or Office
	IsAddressDefault BIT NOT NULL DEFAULT 0,
	-- 0 = True, 1 = False
	-- Constraints
...
(
	...
	-- we need to create a trigger from sub order 
...
(
	...
	-- Constraints
	FOREIGN KEY (LoginName) REFERENCES Seller(LoginName) ON DELETE CASCADE ON UPDATE CASCADE
	-- Connect to Seller
);
GO
...
(
	...
	-- available amount of product in the shop -> use trigger for this
	Weight INT,
	-- Constraints
...
(
	...
	Quantity INT NOT NULL Default 0,
	-- Constraints
	PRIMARY KEY(SubOrderID, ProductID, SKUName),
...
(
	...
	Code AS ('VCH-' + RIGHT('000000' + CAST(VoucherId AS VARCHAR(10)), 6)) PERSISTED PRIMARY KEY,
	-- persisted = saved to disk + indexable
...
AS BEGIN
	...
	WHERE StoredSKU.CartID IN (SELECT CartID
	FROM @ModifiedCart)
	)
END
GO
...
BEGIN
	...
	BEGIN
        ...
        WHERE Cart.CartID IN (SELECT CartID
		FROM @AffectedCarts);
	END
END
GO
...
INSERT INTO UserInfo
	(LoginName, Password, PhoneNumber, Email, UserName, Gender, BirthDate, Address)
VALUES
...
INSERT INTO Seller
	(LoginName, ShopName, CitizenIDCard, SellerName, MoneyEarned)
VALUES
...
INSERT INTO ProductInfo
	(LoginName, ProductName, ProductBrand, ProductCategory, ProductDescription, ProductMadeIn)
VALUES
...
INSERT INTO SKU
	(ProductID, SKUName, Size, Price, InStockNumber, Weight)
VALUES
...
INSERT INTO StoredSKU
	(CartID, ProductID, SKUName, Quantity)
VALUES
	...
	(4, 1, '256GB-Blue', 2),
	-- iPhone 15 Pro (2 units)
	(4, 3, 'Standard-White', 2),
	-- AirPods Pro 2 (2 units)
	...
	(6, 9, '128GB-Beige', 1),
	-- Samsung Tab S9
	(6, 10, 'Standard-Gray', 2),
	-- Logitech Mouse (2 units)
...
VALUES
	...
	('GrabExpress'),
	('Giao Hang Nhanh');
INSERT INTO ProvideDelivery
	(ProviderName, MethodName)
VALUES
	('VNPost', 'Standard'),
	('GrabExpress', 'Express'),
	('Giao Hang Nhanh', 'Economy');
GO
...
BEGIN
	...
	END
		...
		(LoginName, AddressID, ProviderName, AccountID, TotalPrice)
	VALUES
		(@LoginName, @AddressID, @ProviderName, @AccountID, 0);
	...
	WHERE S.CartID = @CartID;
		...
		INSERT INTO SubOrderDetail
			(OrderID, SubOrderID, ProductID, SKUName, Quantity)
		SELECT
			@OrderID,
			@SubOrderID,
			S.ProductID,
	...
	WHERE OrderID = @OrderID
        ...
        UPDATE Buyer
        SET MoneySpent = MoneySpent + (SELECT TotalPrice
	FROM OrderInfo
	WHERE OrderID = @OrderID)
        WHERE LoginName = @LoginName;
	...
	WHERE SOD.OrderID = @OrderID
		...
		(SELECT TotalPrice
		FROM OrderInfo
		WHERE OrderID = @OrderID) AS TotalPrice;
...
INSERT INTO AddressInfo
	(LoginName, ContactName, ContactPhoneNumber, City, District, Commune, DetailAddress, AddressType, IsAddressDefault)
VALUES
	-- user001 (JohnDoe)
